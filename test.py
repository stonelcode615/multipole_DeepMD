...
from deepmd import DeepPrimitiveMoment
from deepmd import DeepMoment
from deepmd import DeepDipolestone
...
def test (args):
    ...
    if de.model_type == 'ener':
        ...
        test_wfc(args)
    elif de.model_type == 'primitivemoment':
        test_primitivemoment(args)
    elif de.model_type == 'moment':
        test_moment(args)
    elif de.model_type == 'dipolestone':
        test_dipolestone(args)
...
def test_primitivemoment (args) :
    if args.rand_seed is not None :
        np.random.seed(args.rand_seed % (2**32))

    dp = DeepPrimitiveMoment(args.model)    
    data = DeepmdData(args.system, args.set_prefix, shuffle_test = args.shuffle_test)
    data.add('primitivemoment', 9, atomic=True, must=True, high_prec=False, type_sel = dp.get_sel_type())
    ...
    box = test_data["box"][:numb_test]
    atype = test_data["type"][0]
    primitivemoment = dp.eval(coord, box, atype)

    primitivemoment = primitivemoment.reshape([numb_test,-1])
    l2f = (l2err (primitivemoment  - test_data["primitivemoment"] [:numb_test]))

    print ("# number of test data : %d " % numb_test)
    print ("PrimitiveMoment  L2err : %e eV/A" % l2f)

    detail_file = args.detail_file
    if detail_file is not None :
        pe = np.concatenate((np.reshape(test_data["primitivemoment"][:numb_test], [-1,9]),
                             np.reshape(primitivemoment, [-1,9])), 
                            axis = 1)
        np.savetxt(detail_file+".out", pe, 
                   header = 'ref_primitivemoment(9 dofs)   predicted_primitivemoment(9 dofs)')

def test_moment (args) :
    if args.rand_seed is not None :
        np.random.seed(args.rand_seed % (2**32))

    dp = DeepMoment(args.model)    
    data = DeepmdData(args.system, args.set_prefix, shuffle_test = args.shuffle_test)
    data.add('moment', 5, atomic=True, must=True, high_prec=False, type_sel = dp.get_sel_type())
    ...
    box = test_data["box"][:numb_test]
    atype = test_data["type"][0]
    moment = dp.eval(coord, box, atype)

    moment = moment.reshape([numb_test,-1])
    l2f = (l2err (moment  - test_data["moment"] [:numb_test]))

    print ("# number of test data : %d " % numb_test)
    print ("Moment  L2err : %e eV/A" % l2f)

    detail_file = args.detail_file
    if detail_file is not None :
        pe = np.concatenate((np.reshape(test_data["moment"][:numb_test], [-1,5]),
                             np.reshape(moment, [-1,5])), 
                            axis = 1)
        np.savetxt(detail_file+".out", pe, 
                   header = 'ref_moment(5 dofs)   predicted_moment(5 dofs)')


def test_dipolestone (args) :
    if args.rand_seed is not None :
        np.random.seed(args.rand_seed % (2**32))

    dp = DeepDipolestone(args.model)    
    data = DeepmdData(args.system, args.set_prefix, shuffle_test = args.shuffle_test)
    data.add('dipolestone', 3, atomic=True, must=True, high_prec=False, type_sel = dp.get_sel_type())
    test_data = data.get_test ()
    numb_test = args.numb_test
    natoms = len(test_data["type"][0])
    nframes = test_data["box"].shape[0]
    ...
    box = test_data["box"][:numb_test]
    atype = test_data["type"][0]
    dipolestone = dp.eval(coord, box, atype)

    dipolestone = dipolestone.reshape([numb_test,-1])
    l2f = (l2err (dipolestone  - test_data["dipolestone"] [:numb_test]))

    print ("# number of test data : %d " % numb_test)
    print ("Dipolestone L2err : %e eV/A" % l2f)

    detail_file = args.detail_file
    if detail_file is not None :
        pe = np.concatenate((np.reshape(test_data["dipolestone"][:numb_test], [-1,3]),
                             np.reshape(dipolestone, [-1,3])), 
                            axis = 1)
        np.savetxt(detail_file+".out", pe, 
                   header = 'ref_dipolestone(3 dofs)   predicted_dipolestone(3 dofs)')
