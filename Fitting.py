...
class PrimitiveMomentFitting () :
    def __init__ (self, jdata, descrpt) :
        if not isinstance(descrpt, DescrptLocFrame) :
            raise RuntimeError('PrimitiveMoment only supports DescrptLocFrame')
        self.ntypes = descrpt.get_ntypes()
        self.dim_descrpt = descrpt.get_dim_out()
        args = ClassArg()\
               .add('neuron',           list,   default = [120,120,120], alias = 'n_neuron')\
               .add('resnet_dt',        bool,   default = True)\
               .add('primitivemoment_numb',         int,    must = True)\
               .add('sel_type',         [list,int],   default = [ii for ii in range(self.ntypes)], alias = 'primitivemoment_type')\
               .add('seed',             int)
        class_data = args.parse(jdata)
        self.n_neuron = class_data['neuron']
        self.resnet_dt = class_data['resnet_dt']
        self.primitivemoment_numb = class_data['primitivemoment_numb']
        self.sel_type = class_data['sel_type']
        self.seed = class_data['seed']
        self.useBN = False
    ...
    def get_primitivemoment_numb(self):
        return self.primitivemoment_numb

    def get_out_size(self):
        return self.primitivemoment_numb * 9
        ...
        for type_i in range(self.ntypes):
            ...
            for ii in range(0,len(self.n_neuron)) :
                if ii >= 1 and self.n_neuron[ii] == self.n_neuron[ii-1] :
                    ...
                    layer = one_layer(layer, self.n_neuron[ii], name='layer_'+str(ii)+'_type_'+str(type_i)+suffix, reuse=reuse, seed = self.seed)
            # (nframes x natoms) x (nmoment x 9)
            final_layer = one_layer(layer, self.primitivemoment_numb * 9, activation_fn = None, name='final_layer_type_'+str(type_i)+suffix, reuse=reuse, seed = self.seed)
            # (nframes x natoms) x 3 x 3
            final_layer = tf.reshape(final_layer, [tf.shape(inputs)[0] * natoms[2+type_i], 3, 3])
            # (nframes x natoms) x 3 x 3
            final_layer = 0.5 * (final_layer + tf.transpose(final_layer, perm = [0,2,1]))
            # (nframes x natoms) x 3 x 3
            final_layer = tf.matmul(final_layer, rot_mat_i) #stone
            # (nframes x natoms) x 3(coord) x 3(coord)
            final_layer = tf.matmul(rot_mat_i, final_layer, transpose_a = True)
            # nframes x natoms x nmoment x 3 x 3
            final_layer = tf.reshape(final_layer, [tf.shape(inputs)[0], natoms[2+type_i], 3, 3])
...
class MomentFitting () :
    def __init__ (self, jdata, descrpt) :
        if not isinstance(descrpt, DescrptLocFrame) :
            raise RuntimeError('Moment only supports DescrptLocFrame')
        self.ntypes = descrpt.get_ntypes()
        self.dim_descrpt = descrpt.get_dim_out()
        args = ClassArg()\
               .add('neuron',           list,   default = [120,120,120], alias = 'n_neuron')\
               .add('resnet_dt',        bool,   default = True)\
               .add('moment_numb',         int,    must = True)\
               .add('sel_type',         [list,int],   default = [ii for ii in range(self.ntypes)], alias = 'moment_type')\
               .add('seed',             int)
        class_data = args.parse(jdata)
        self.n_neuron = class_data['neuron']
        self.resnet_dt = class_data['resnet_dt']
        self.moment_numb = class_data['moment_numb']
        self.sel_type = class_data['sel_type']
        self.seed = class_data['seed']
        self.useBN = False
    ...
    def get_moment_numb(self):
        return self.moment_numb

    def get_out_size(self):
        return self.moment_numb * 5
        ...
        for type_i in range(self.ntypes):
            ...
            for ii in range(0,len(self.n_neuron)) :
                if ii >= 1 and self.n_neuron[ii] == self.n_neuron[ii-1] :
                    ...
                    layer = one_layer(layer, self.n_neuron[ii], name='layer_'+str(ii)+'_type_'+str(type_i)+suffix, reuse=reuse, seed = self.seed)
            # (nframes x natoms) x (nmoment x 5)
            final_layer = one_layer(layer, self.moment_numb * 5, activation_fn = None, name='final_layer_type_'+str(type_i)+suffix, reuse=reuse, seed = self.seed)
            # (nframes x natoms) x nmoment x 5(element_local)
            final_layer = tf.reshape(final_layer, [tf.shape(inputs)[0] * natoms[2+type_i], self.moment_numb, 5])
            # (nframes x natoms) x nmoment x 5(element)
            #final_layer = tf.matmul(final_layer, rot_mat_i) #stone
            # nframes x natoms x nmoment x 5(element_local)
            final_layer = tf.reshape(final_layer, [tf.shape(inputs)[0], natoms[2+type_i], self.moment_numb, 5])
...
class DipolestoneFitting () :
    def __init__ (self, jdata, descrpt) :
        if not isinstance(descrpt, DescrptLocFrame) :
            raise RuntimeError('dipolestone only supports DescrptLocFrame')
        self.ntypes = descrpt.get_ntypes()
        self.dim_descrpt = descrpt.get_dim_out()
        args = ClassArg()\
               .add('neuron',           list,   default = [120,120,120], alias = 'n_neuron')\
               .add('resnet_dt',        bool,   default = True)\
               .add('dipolestone_numb', int,    must = True)\
               .add('sel_type',         [list,int],   default = [ii for ii in range(self.ntypes)], alias = 'dipolestone_type')\
               .add('seed',             int)
        class_data = args.parse(jdata)
        self.n_neuron = class_data['neuron']
        self.resnet_dt = class_data['resnet_dt']
        self.dipolestone_numb = class_data['dipolestone_numb']
        self.sel_type = class_data['sel_type']
        self.seed = class_data['seed']
        self.useBN = False
    ...
    def get_dipolestone_numb(self):
        return self.dipolestone_numb

    def get_out_size(self):
        return self.dipolestone_numb * 3
        ...
        for type_i in range(self.ntypes):
            ...
            for ii in range(0,len(self.n_neuron)) :
                if ii >= 1 and self.n_neuron[ii] == self.n_neuron[ii-1] :
                    ...
                    layer = one_layer(layer, self.n_neuron[ii], name='layer_'+str(ii)+'_type_'+str(type_i)+suffix, reuse=reuse, seed = self.seed)
            # (nframes x natoms) x (ndipolestone x 3)
            final_layer = one_layer(layer, self.dipolestone_numb * 3, activation_fn = None, name='final_layer_type_'+str(type_i)+suffix, reuse=reuse, seed = self.seed)
            # (nframes x natoms) x ndipolestone x 3(element_local)
            final_layer = tf.reshape(final_layer, [tf.shape(inputs)[0] * natoms[2+type_i], self.dipolestone_numb, 3])
            # (nframes x natoms) x ndipolestone x 3(element)
            final_layer = tf.matmul(final_layer, rot_mat_i) #stone
            # nframes x natoms x ndipolestone x 3(element_local)
            final_layer = tf.reshape(final_layer, [tf.shape(inputs)[0], natoms[2+type_i], self.dipolestone_numb, 3])
