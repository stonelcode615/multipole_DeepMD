...
from deepmd.Fitting import EnerFitting, WFCFitting, PrimitiveMomentFitting, MomentFitting, DipolestoneFitting, PolarFittingLocFrame, PolarFittingSeA, GlobalPolarFittingSeA, DipoleFittingSeA
from deepmd.DescrptLocFrame import DescrptLocFrame
from deepmd.DescrptSeA import DescrptSeA
from deepmd.DescrptSeR import DescrptSeR
from deepmd.DescrptSeAR import DescrptSeAR
from deepmd.Model import Model, WFCModel, DipoleModel, PolarModel, GlobalPolarModel,DipolestoneModel,MomentModel, PrimitiveMomentModel
...
class NNPTrainer (object):
    ...
    def _init_param(self, jdata):
        ...
        if fitting_type == 'ener':
            ...
            self._message('stone fitting net init')
        elif fitting_type == 'primitivemoment':            
            self.fitting = PrimitiveMomentFitting(fitting_param, self.descrpt)
            self._message('stone fitting net init')
        elif fitting_type == 'moment':            
            self.fitting = MomentFitting(fitting_param, self.descrpt)
            self._message('stone fitting net init')
        elif fitting_type == 'dipolestone':            
            self.fitting = DipolestoneFitting(fitting_param, self.descrpt)
            self._message('stone fitting net init')
        ...
        if fitting_type == Model.model_type:
            ...
            self._message('stone PrimitiveMomentmodel init\n')
            self.model = PrimitiveMomentModel(model_param, self.descrpt, self.fitting)
        elif fitting_type == 'moment':
            self._message('stone Momentmodel init\n')
            self.model = MomentModel(model_param, self.descrpt, self.fitting)
        elif fitting_type == 'dipolestone':
            self._message('stone dipolestone Model init\n')
            self.model = DipolestoneModel(model_param, self.descrpt, self.fitting)       
        elif fitting_type == 'dipole':
        ...
        if fitting_type == 'ener':
            ...
            self._message("stone TensorLoss init\n")
            self.loss = TensorLoss(loss_param, 
                                   model = self.model, 
                                   tensor_name = 'primitivemoment',
                                   tensor_size = self.model.get_out_size(),
                                   label_name = 'primitivemoment')
        elif fitting_type == 'moment':
            self._message("stone TensorLoss init\n")
            self.loss = TensorLoss(loss_param, 
                                   model = self.model, 
            ...
            self.loss = TensorLoss(loss_param, 
                                   model = self.model, 
                                   tensor_name = 'dipolestone',
                                   tensor_size = self.model.get_out_size(),
                                   label_name = 'dipolestone')
        elif fitting_type == 'dipole':
            self.loss = TensorLoss(loss_param, 
                                   model = self.model, 
                                   tensor_name = 'dipole',
